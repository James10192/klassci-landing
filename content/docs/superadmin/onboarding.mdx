...
![Formulaire Nouvelle Unité d'Enseignement avec sections Identité (Intitulé + Code), Paramètres académiques (Crédits + Type UE) et Description](/img/docs/superadmin/onboarding/19-ue-formulaire.png)
...
![Ligne d'UE avec les boutons d'action : ajouter ECUE (vert), lier aux parcours (route), modifier, supprimer](/img/docs/superadmin/onboarding/20-ue-actions-row.png)

Boutons sur la ligne, de gauche à droite : <Icon name="plus-circle" className="text-emerald-500" /> **vert** ajouter un ECUE, **route** indigo lier l'UE à des parcours, <Icon name="pencil" /> modifier l'UE, <Icon name="trash" /> supprimer. *(Pour déplier la liste des ECUE déjà rattachés, cliquez directement sur la ligne UE — la flèche `▶` qui apparaît à gauche n'est pas un bouton à cliquer mais un indicateur visuel.)*

Cliquez sur le **bouton vert <Icon name="plus-circle" className="text-emerald-500" />** pour ouvrir le formulaire d'ajout d'ECUE.
...
→ [**Module Frais & Comptabilité**](/fr/docs/modules/frais-comptabilite)
...
Une fois fait, l'équipe comptable prend le relais via [Comptable — opérations](/fr/docs/comptable/operations) pour le quotidien (paiements, relances, exports).
...
- Pour la gestion quotidienne des inscriptions, lisez le guide [Secrétaire — gérer les inscriptions](/fr/docs/secretaire/inscriptions).
