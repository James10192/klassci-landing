...
→ [**Module Frais & Comptabilité**](/docs/modules/frais-comptabilite)
...
Une fois fait, l'équipe comptable prend le relais via [Comptable — opérations](/docs/comptable/operations) pour le quotidien (paiements, relances, exports).
...
![Sidebar avec la section Administration ouverte montrant Personnel et Gestion du personnel](/img/docs/superadmin/onboarding/35-sidebar-personnel.png)
...
![Page Gestion du Personnel avec stats par rôle et 5 onglets](/img/docs/superadmin/onboarding/36-personnel-liste.png)
...
![Formulaire Créer un Coordinateur avec sections Informations personnelles, Adresse, Sécurité et Privilèges](/img/docs/superadmin/onboarding/37-coordinateur-form.png)
...
![Wizard étape 1 : Nom complet, Email, Téléphone, Titre académique](/img/docs/superadmin/onboarding/39-enseignant-wizard-1.png)

Nom complet *(requis)*, Email, Téléphone, **Titre académique** (Pr., Dr., M., Mme…). Bouton **Suivant**.

**Étape 2/4 — Qualifications & Expérience**

![Wizard étape 2 : Diplôme principal, Université, Année d'obtention, Grade académique, Années d'expérience, Spécialisation](/img/docs/superadmin/onboarding/40-enseignant-wizard-2.png)
...
![Wizard étape 3 : Département, Type de contrat, Statut d'emploi, Date d'embauche, Taux horaire, Charge horaire max](/img/docs/superadmin/onboarding/41-enseignant-wizard-3.png)
...
![Wizard étape 4 : Biographie, Motivation, Objectifs pédagogiques, Site web, CV upload, Photo upload (tous optionnels)](/img/docs/superadmin/onboarding/42-enseignant-wizard-4.png)
...
![Modale Compte créé avec succès affichant le nom d'utilisateur et le mot de passe temporaire](/img/docs/superadmin/onboarding/38-personnel-credentials.png)

Le **nom d'utilisateur** suit un préfixe par rôle (`coord.prenom.nom`, `prof.prenom.nom`, `sec.prenom.nom`…) avec déduplication automatique (`.1`, `.2`…) en cas de collision. Le **mot de passe temporaire** est sécurisé et affiché en clair une seule fois — utilisez le bouton **Imprimer** ou le bouton de copie pour le transmettre au membre. Pour les enseignants, un bouton **Voir la fiche** mène directement à la fiche détaillée pour configurer la disponibilité.

À la **première connexion**, le membre est forcé de changer son mot de passe. Les mots de passe expirent au bout de **6 mois** avec une alerte 1 mois avant.
...
2. **Créer un étudiant test** — `Étudiants → Nouvel étudiant`. Inscrivez-le dans une classe. Vérifiez que la classe accepte l'inscription (capacité non atteinte, année courante, filière active).
